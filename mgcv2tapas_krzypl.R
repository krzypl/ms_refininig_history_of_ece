#mgcv2tapas function applied to gam output gives the error: "Error in `stringr::str_replace()`: ! `string` must be a vector, not a symbol". Therefore, the function was modified by replacing the part containing the str_replace() function with a defined string

mgcv2tapas_krzypl <- function (series = NULL, data_type = "accI", series.name = NA, 
                               plotit = TRUE) 
{
  ...
  age_bot <- d_raw %>% dplyr::pull(var = 4)
  volI <- d_raw %>% dplyr::pull(var = 5)
  ...
  if (data_type == "accI" | data_type == "acc") {
    ...
    y_name <- "sand" #this part was modified from stringr::str_replace(y_var_name, "AR", "")
  }
